    ...
    switch (req.method) {
      case "GET": {
        ...
        const icons = (productManager.icon || []).map((filename) => ({
          filename,
          url: `${process.env.NEXTAUTH_URL}/api/files/${encodeURIComponent(
            filename
          )}`,
        }));
        ...
        const processTableCellData = (data: any): TableCellDataFrontend[] => {
          ...
          return data
            .map((item: any): TableCellDataFrontend | null => {
              if (
                item &&
                typeof item === "object" &&
                ...
                console.warn(
                  "GET: Invalid tableCellData item structure from DB:",
                  item
                );
                return null;
              }
        ...
        const tableCellData = processTableCellData(
        ...
        const processGlobalVariableClassData = (
          data: any[] | undefined
        ): any[] => {
          if (!Array.isArray(data)) {
            return [];
          ...
          return data
            .map((item, index) => {
              if (typeof item !== "object" || item === null) {
                console.warn(
                  `Invalid item found in globalVariableClassData at index ${index}:`,
                  item
                );
                return {
                  dataId: -1,
              ...
              return {
                ...
                variableData:
                  typeof variableDataFromDB === "object" &&
                  variableDataFromDB !== null
                    ? variableDataFromDB
                    : {},
              };
            })
        ...
        const globalVariableClassData = processGlobalVariableClassData(
        ...
        const processGlobalVariablePackageData = (
          data: any[] | undefined
        ): any[] => {
          if (!Array.isArray(data)) {
            return [];
          ...
          return data
            .map((item, index) => {
              ...
              if (
                ...
                processedVariableData = Object.entries(
                  variableDataFromDB
                  ...
                  if (entry && typeof entry === "object" && "value" in entry) {
                    ...
                    if (typeof valueFromDB === "string") {
                      try {
                        ...
                        if (
                          ...
                          console.warn(
                            `Parsed value for package item ${item.dataId}, key ${key}, is not the expected object structure. Keeping original string. Parsed:`,
                            parsedValue
                          );
                        ...
                        console.error(
                          `Failed to parse value for package item ${item.dataId}, key ${key}. Keeping original string. Error:`,
                          parseError,
                          `Value:`,
                          valueFromDB
                        );
                      ...
                      typeof valueFromDB === "object" &&
                      valueFromDB !== null
                    ) {
                      acc[key] = { ...entry, value: valueFromDB };
                    } else {
                      console.warn(
                        `Unexpected value type for package item ${item.dataId}, key ${key}:`,
                        valueFromDB
                      );
                      acc[key] = { ...entry, value: valueFromDB };
                    }
                    ...
                    console.warn(
                      `Invalid entry structure (missing 'value'?) for package item ${item.dataId}, key ${key}:`,
                      entry
                    );
                    acc[key] = entry;
                  }
                ...
                console.warn(
                  `variableData for package item ${item.dataId} is not a valid object:`,
                  variableDataFromDB
                );
              }
        ...
        const globalVariablePackageData = processGlobalVariablePackageData(
      ...
      case "PATCH": {
        ...
        try {
          ...
          try {
            if (
              formFields.existingIconsJson &&
              typeof formFields.existingIconsJson === "string"
            ) {
              existingIcons = JSON.parse(formFields.existingIconsJson);
              if (!Array.isArray(existingIcons)) {
                console.warn(
                  "Parsed existingIconsJson was not an array, defaulting to empty."
                );
                existingIcons = [];
              }
            } else {
          ...
          try {
            if (
              formFields.existingPdfsJson &&
              typeof formFields.existingPdfsJson === "string"
            ) {
              existingPDFs = JSON.parse(formFields.existingPdfsJson);
              if (!Array.isArray(existingPDFs)) {
                console.warn(
                  "Parsed existingPdfsJson was not an array, defaulting to empty."
                );
                existingPDFs = [];
              }
            } else {
          ...
          const uploadedIconFiles = allUploadedFiles.filter(
            (file: Express.Multer.File) => file.mimetype.startsWith("image/")
          );
          const uploadedPdfFiles = allUploadedFiles.filter(
          ...
          const newIcons = await Promise.all(
            uploadedIconFiles.map(async (file: Express.Multer.File) => {
              ...
              const existingGridFSFiles = await bucket
                .find({
                  filename,
              ...
              if (existingGridFSFiles.length > 0) {
                await Promise.all(
                  existingGridFSFiles.map((f) => bucket.delete(f._id))
                );
              }
          ...
          if (uniqueIcons.length > 0) {
            ...
            updateData.iconPreview = uniqueIcons.map(
              (filename) =>
                `${process.env.NEXTAUTH_URL}/api/files/${encodeURIComponent(
                  filename
                )}`
            );
          } else {
          ...
          if (uniquePdfs.length > 0) {
            ...
            updateData.pdfPreview = uniquePdfs.map(
              (filename) =>
                `${process.env.NEXTAUTH_URL}/api/files/${encodeURIComponent(
                  filename
                )}`
          ...
          const approvedTableCellClearFlag =
            req.body.approvedTableCellClear === "true";
          ...
          let incomingTableCellDataItems: ProcessedCellData[] | undefined =
            undefined;
          ...
          if (tableCellDataFromBody === undefined) {
            ...
            console.error(
              "Received tableCellData is not an array. Skipping update.",
            ...
            tableCellDataFromBody.length === 1 &&
            tableCellDataFromBody[0] === "[]"
          ) {
            console.warn(
              "Received legacy '[]' string marker, treating as empty."
            );
            updateData.tableCellData = [];
          } else if (tableCellDataFromBody.length % 6 !== 0) {
            console.error(
              `Received tableCellData with incorrect number of items (${tableCellDataFromBody.length}), expected sextuplets (index, key, value, isComposite, isPackage, isDisabled). Skipping update.`,
              tableCellDataFromBody
            );
            processingError = true;
          } else {
            ...
            for (let i = 0; i < tableCellDataFromBody.length; i += 6) {
              ...
              if (
                ...
                typeof isCompositeStr !== "string" ||
                typeof isPackageStr !== "string" ||
                typeof isDisabledStr !== "string"
              ) {
                console.warn(`Malformed item at index ${i / 6}. Skipping.`, {
                  indexStr,
                  classKey,
                  valueStr,
                  isCompositeStr,
              ...
              if (isComposite) {
                try {
                  ...
                  if (Array.isArray(parsedArray)) {
                    ...
                    console.warn(
                      `Composite value for ${classKey}[${index}] is not a valid JSON array, storing as string:`,
                      valueStr
                    );
                  }
                } catch (e) {
                  console.warn(
                    `Failed to parse composite value for ${classKey}[${index}] as JSON array, storing as string:`,
                    valueStr,
                    e
                  );
                }
          ...
          if (incomingTableCellDataItems && !processingError) {
            let existingTableCellData: ProcessedCellData[] = (
              productManager.tableCellData || []
            )
              .map(
                (item: any): ProcessedCellData => ({
                  index: item?.index ?? -1,
                  classKey: item?.classKey ?? "",
                  value: item?.value,
            ...
            const normalizeData = (
              data: ProcessedCellData | undefined
            ): ProcessedCellData => {
              if (!data)
                return {
                  classKey: "",
            ...
            const areItemsEqual = (
              item1: ProcessedCellData,
              item2: ProcessedCellData
            ): boolean => {
              if (
                item1.classKey !== item2.classKey ||
            ...
            const hasDataChanges =
              dataToDelete.length > 0 || dataToAdd.length > 0;
            ...
            console.error(
              "Skipping tableCellData update due to processing errors."
            );
          }

          let tableSheetDataFromBody = req.body.tableSheet;
          const approvedTableSheetClearFlag =
            req.body.approvedTableSheetClear === "true";
          ...
          if (Array.isArray(tableSheetDataFromBody)) {
            if (tableSheetDataFromBody.length === 0) {
              ...
              if (tableSheetDataFromBody.length % 3 !== 0) {
                console.error(
                  "Received tableSheet with incorrect number of items, expected triplets. Skipping update.",
                  tableSheetDataFromBody
                );
              } else {
              ...
              existingTableSheet = existingTableSheet.map(
                (item: any, idx: number) => {
              ...
              const newOriginKey = tableSheetKeys.find(
                (key: any) => key.isOrigin === true
              );
              const existingOriginKey = existingTableSheet.find(
                (key: any) => key.isOrigin === true
              );

              const normalizeKey = (
                key: any
              ): { index: number; value: string; isOrigin: boolean } => {
              ...
              const normalizeKeyForComparison = (
                key: any
              ): { value: string; isOrigin: boolean } => {
              ...
              const normalizedExistingKeys =
                existingTableSheet.map(normalizeKey);
              const normalizedFrontendKeys = tableSheetKeys.map(normalizeKey);

              const existingKeySet = new Set(
                normalizedExistingKeys.map((k: any) => k.value)
              );
              const frontendKeySet = new Set(
                normalizedFrontendKeys.map((k) => k.value)
              );

              const keysToDelete = normalizedExistingKeys.filter(
                (dbKey: any) => !frontendKeySet.has(dbKey.value)
              );

              const keysToAdd = normalizedFrontendKeys.filter(
                (frontendKey) => !existingKeySet.has(frontendKey.value)
              );
              ...
              for (const frontendKey of normalizedFrontendKeys) {
                ...
                const dbKey = normalizedExistingKeys.find(
                  (k: any) => k.value === frontendKey.value
                );
                if (!dbKey) continue;
              ...
              function areArraysEquivalentIgnoringIndex(
                arr1: any[],
                arr2: any[]
              ): boolean {
                if (!arr1 && !arr2) return true;
              ...
              const arraysEquivalent = areArraysEquivalentIgnoringIndex(
                existingTableSheet,
                tableSheetKeys
              );
              if (arraysEquivalent) {
              ...
              if (!arraysEquivalent) {
                const set1 = new Set(
                  existingTableSheet.map(getComparisonSignature)
                );
                const set2 = new Set(
                  tableSheetKeys.map(getComparisonSignature)
                );
              ...
              if (hasClassKeyChanges || originHasChanged) {
                ...
                if (originHasChanged) {
                  ...
                  if (
                    ...
                    console.error(
                      "CRITICAL WARNING: About to delete all keys with no replacements"
                    );
                    console.error(
                      "This looks like a data loss scenario, aborting operation"
                    );

                    return res.status(400).json({
                      error:
                        "Prevented potential data loss. Attempted to delete all keys without replacement.",
                    });
                  }

                  const existingKeysToKeep = existingTableSheet.filter(
                    (dbKey: any) =>
                      !keysToDelete.some(
                        (key: any) => key.value === dbKey.value
                      ) &&
                  ...
                  const updatedKeys = keysToUpdate.map((frontendKey: any) => {
                    const dbKey = existingTableSheet.find(
                      (key: any) => key.value === frontendKey.value
                    );
                    return {
          ...
          const approvedVariableClassClearFlag =
            formFields.approvedGlobalVariableClassClear === "true";
          ...
          if (
            rawGlobalVariableClassData !== undefined &&
            typeof rawGlobalVariableClassData === "string"
          ) {
            try {
              const parsedData = JSON.parse(rawGlobalVariableClassData);
              if (Array.isArray(parsedData)) {
                incomingVariableClassData = parsedData
                  .map((item: any, index: number) => {
                    if (
                      ...
                      console.warn(
                        `Invalid item structure in globalVariableClassData at index ${index}. Skipping item. Item:`,
                        item
                      );
                ...
                if (variableClassParseError) {
                  console.warn(
                    "Found invalid items during globalVariableClassData parsing. Update for this field will be skipped."
                  );
                ...
                console.error(
                  "Parsed globalVariableClassData is not an array. Skipping update for this field."
                );
                variableClassParseError = true;
              }
            } catch (jsonParseError) {
              console.error(
                "Failed to parse globalVariableClassData JSON string:",
                jsonParseError
              );
              variableClassParseError = true;
            }
            ...
            console.error(
              `Unexpected type for globalVariableClassData: ${typeof rawGlobalVariableClassData}. Expected a JSON string. Skipping update for this field.`
            );
            variableClassParseError = true;
          } else {
          }
          ...
          if (!variableClassParseError) {
            ...
            const existingVariableClassData = ensureArray(
              productManager.globalVariableClassData
            );
            ...
            if (incomingVariableClassData === undefined) {
              if (
                approvedVariableClassClearFlag &&
                existingVariableClassData.length > 0
              ) {
              ...
              const currentIncomingVariableClassData = ensureArray(
                incomingVariableClassData
              );
              const sortById = (a: any, b: any) =>
                (a?.dataId ?? 0) - (b?.dataId ?? 0);

              const sortedExisting = [...existingVariableClassData].sort(
                sortById
              );
              const sortedIncoming = [...currentIncomingVariableClassData].sort(
                sortById
              );
            ...
            console.warn(
              "Skipping globalVariableClassData update due to previous parsing or validation errors."
            );
          }

          const approvedVariablePackageClearFlag =
            formFields.approvedGlobalVariablePackageClear === "true";
          let incomingVariablePackageData: any[] | undefined = undefined;
          let variablePackageParseError = false;

          const rawGlobalVariablePackageData =
            formFields.globalVariablePackageData;

          if (
            rawGlobalVariablePackageData !== undefined &&
            typeof rawGlobalVariablePackageData === "string"
          ) {
            try {
              const parsedData = JSON.parse(rawGlobalVariablePackageData);
              if (Array.isArray(parsedData)) {
                incomingVariablePackageData = parsedData
                  .map((item: any, index: number) => {
                    if (
                      typeof item === "object" &&
                      item !== null &&
                      ...
                      console.warn(
                        `Invalid item structure in globalVariablePackageData at index ${index}. Skipping item. Item:`,
                        item
                      );
                ...
                if (variablePackageParseError) {
                  console.warn(
                    "Found invalid items during globalVariablePackageData parsing. Update for this field will be skipped."
                  );
                ...
                console.error(
                  "Parsed globalVariablePackageData is not an array. Skipping update for this field."
                );
                variablePackageParseError = true;
              }
            } catch (jsonParseError) {
              console.error(
                "Failed to parse globalVariablePackageData JSON string:",
                jsonParseError
              );
              variablePackageParseError = true;
            }
            ...
            console.error(
              `Unexpected type for globalVariablePackageData: ${typeof rawGlobalVariablePackageData}. Expected a JSON string. Skipping update for this field.`
            );
            variablePackageParseError = true;
          } else {
          }
          ...
          if (!variablePackageParseError) {
            ...
            const existingVariablePackageData = ensureArray(
              productManager.globalVariablePackageData
            );
            ...
            if (incomingVariablePackageData === undefined) {
              if (
                approvedVariablePackageClearFlag &&
                existingVariablePackageData.length > 0
              ) {
              ...
              const currentIncomingVariablePackageData = ensureArray(
                incomingVariablePackageData
              );
              const sortById = (a: any, b: any) =>
                (a?.dataId ?? 0) - (b?.dataId ?? 0);

              const sortedExisting = [...existingVariablePackageData].sort(
                sortById
              );
              const sortedIncoming = [
                ...currentIncomingVariablePackageData,
              ].sort(sortById);
            ...
            console.warn(
              "Skipping globalVariablePackageData update due to previous parsing or validation errors."
            );
          }
          ...
          if (!updatedManager) {
            return res
              .status(404)
              .json({
                error: "Product manager not found after update attempt",
              });
          }
          ...
          const responsePayload = {
            ...
            globalVariableClassData:
              updatedManager.globalVariableClassData || [],
            globalVariablePackageData:
              updatedManager.globalVariablePackageData || [],
      ...
      case "DELETE": {
        ...
        res
          .status(200)
          .json({ message: "Product manager deleted successfully" });
        break;
      }
      default:
