...
import ZoomControls from "./utility/ZoomControls";
...
interface TableProps {
    ...
    setVariableClassData: React.Dispatch<
...
const Table: React.FC<TableProps> = ({
    ...
    const systemLogSave = useSelector(
        (state: RootState) => state.systemLogger.saved
    );
    const systemLogCurrent = useSelector(
        (state: RootState) => state.systemLogger.current
    );
    const systemLogFailed = useSelector(
        (state: RootState) => state.systemLogger.failed
    );
    const systemLogBatch = useSelector(
        (state: RootState) => state.systemLogger.batch
    );

    const [editingCellKey, setEditingCellKey] = useState<string | null>(null);
    const [editingCellValue, setEditingCellValue] = useState<string>("");
    const inlineInputRef = useRef<HTMLInputElement>(null);
    const tableRowElementsRef = useRef<(HTMLTableRowElement | null)[]>([]);
    ...
    const [actionTargetCell, setActionTargetCell] = useState<{
        key: string;
        rowIndex: number;
        value: string | string[] | IGlobalVariablePackage;
        isComposite: boolean;
        isPackage: boolean;
    } | null>(null);
    const [currentAction, setCurrentAction] = useState<
        "extendLength" | "fillEmpty" | null
    >(null);
    ...
    const [targetImportColumn, setTargetImportColumn] = useState<string | null>(
        null
    );
    ...
    useEffect(() => {
        ...
        const duplicate = classKeyInputObjects.some(
            (keyObj) => keyObj.value === localClassKeyInput.trim()
        );
    ...
    useEffect(() => {
        if (tableRef.current) {
            tableRef.current.style.transform = `scale(${zoomLevel / 100})`;
    ...
    const classKeyInputObjects =
        addedClassKeys.length > 0
            ? addedClassKeys
            : processTableSheet(variableData);
    ...
    const handleImportRowSheet = async (
        ...
        try {
            ...
            reader.onload = (loadEvent) => {
                ...
                if (fileContent) {
                    const rows = (fileContent as string)
                        .split("\n")
                        .map((row) => row.trim())
                        .filter((row) => row);
                    ...
                    if (rows.length > 0) {
                        ...
                        toast.error(
                            "The uploaded file is empty or contains only empty rows."
                        );
                    }
                } else {
            ...
            console.error(
                `Error importing row data for ${targetImportColumn}:`,
                error
            );
    ...
    const handleDeleteKey = (key: string) => {
        const updatedKeys = classKeyInputObjects.filter(
        ...
        setVariableClassData((prevData: VariableRowDataState) => {
            ...
            if (
                ...
                Object.keys(updatedData).every(
                    (dk) => !dk.startsWith(`${columnKey}_row_`)
                )
            ) {
        ...
        if (confirmation) {
            setVariableClassData((prevData: VariableRowDataState) => {
                ...
                Object.keys(updatedData).forEach((dataKey) => {
                    if (
                        dataKey === columnKey ||
                        dataKey.startsWith(`${columnKey}_row_`)
                    ) {
                        delete updatedData[dataKey];
                        clearedCount++;
                    }
    ...
    const handleSetDefaultColumnValue = (columnKey: string) => {
        const defaultValue = prompt(
            `Enter default value for column "${columnKey}":`
        );
        ...
        if (defaultValue.trim() === "") {
            toast.warn(
                "Default value cannot be empty. Use 'Clear' button to remove values."
            );
            return;
        }
        ...
        setVariableClassData((prevData) => {
            ...
            for (let i = 0; i <= highestRowToFill; i++) {
                ...
                if (shouldApplyDefault) {
                    ...
                    existingCell &&
                    existingCell.isDefault &&
                    existingCell.value !== defaultValue
                ) {
                    updates[dataKey] = {
                        ...existingCell,
                        value: defaultValue,
            ...
            if (defaultAppliedCount > 0) {
                ...
                toast.info(
                    `No empty cells found to apply default value in column "${columnKey}". Existing values were preserved.`
                );
                return prevData;
            }
        });
    ...
    const actionExtendLength = () => {
        ...
        const logValue = isTargetIsPackage
            ? `Package (ID: ${(valueToExtend as unknown as IGlobalVariablePackage)?.dataId
            })`
            : valueToExtend;
    ...
    const actionFillEmpty = () => {
        ...
        const logValue = isTargetIsPackage
            ? `Package (ID: ${(valueToFill as unknown as IGlobalVariablePackage)?.dataId
            })`
            : valueToFill;
        ...
        for (let i = rowIndex + 1; i <= maxRowIndex; i++) {
            ...
            let isCellEmpty =
                !existingCell ||
                (existingCell.isComposite &&
                    Array.isArray(existingCell.value) &&
                    existingCell.value.length === 0) ||
                (!existingCell?.isComposite &&
                    !existingCell?.isPackage &&
                    typeof existingCell?.value === "string" &&
                    existingCell.value.trim() === "") ||
                (!existingCell?.isComposite &&
                    !existingCell?.isPackage &&
    ...
    const renderActionModal = () => {
        ...
        const displayValueInModal =
            isComposite && Array.isArray(value)
        ...
        if (currentAction === "extendLength") {
            return (
                <div className={styles.actionModal}>
                    <h3>
                        Extend Value for "{key}" (Row {rowIndex + 1})
                    </h3>
                    <p>
                        Value: <strong>{displayValueInModal}</strong>
                    </p>
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
        ...
        if (currentAction === "fillEmpty") {
            return (
                <div className={styles.actionModal}>
                    <h3>
                        Fill Empty Below for "{key}" (Row {rowIndex + 1})
                    </h3>
                    <p>
                        This will fill all empty cells below Row {rowIndex + 1} in the "
                        {key}" column with the value:
                    </p>
                    <p>
                        <strong>{displayValueInModal}</strong>
                    </p>
                    <div className={styles.actionModalButtons}>
                        <button
                            type="button"
                            onClick={actionFillEmpty}
        ...
        return (
            <div className={styles.actionModal}>
                <h3>
                    Cell Actions for "{key}" (Row {rowIndex + 1})
                </h3>
                <p>
                    Value: <strong>{displayValueInModal}</strong>
                </p>
                <div className={styles.actionModalButtons}>
                    <button onClick={() => setCurrentAction("extendLength")}>
                        Extend Value Down...
                    </button>
                    <button onClick={() => setCurrentAction("fillEmpty")}>
        ...
        if (confirmation) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    console.error(
                        `Failed to clear table data on server. Status: ${response.status}`,
                        errorData
                    );
                    toast.error(
                        `Error clearing table data: ${errorData.error || errorData}`
                    );
                }
            } catch (error) {
    ...
    const handleMinimapRowClick = (rowIndex: number) => {
        if (tableRowElementsRef.current[rowIndex]) {
            tableRowElementsRef.current[rowIndex]?.scrollIntoView({
                behavior: "smooth",
                block: "nearest",
            });
        }
    ...
    return (
        <div className={styles.wavekeyTable}>
            <div className={styles.header}>
                <h2 onClick={(e) => handleDisplayTable(e)}>
                    Table |{" "}
                    <span style={{ fontWeight: "10", fontSize: "12pt" }}>
                        {`${Object.keys(variableRowData).length} total queries`} |{" "}
                        {hideTable ? "Show" : "Hide"}
                    </span>
                </h2>
                {!hideTable && (
            ...
            {actionModalOpen && (
                <div className={styles.actionModalOverlay}>{renderActionModal()}</div>
            )}
            {!hideTable && (
                <div className={styles.tableLayoutContainer}>
                    ...
                    <div
                        className={
                            styles.tableDisplay +
                            `${hideTable ? styles.closeTableDisplay : styles.openTableDisplay
                            }`
                        }
                    >
                        <form>
                            ...
                            <div className={styles.rowContainer}>
                                    ...
                                    <div className={styles.classKeyButtons}>
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                areRowsPopulated(false);
                                                setApprovedTableSheetClear &&
                                                    setApprovedTableSheetClear(true);
                                            }}
                                ...
                                {headerOrigin && !permanentOrigin ? (
                                    ...
                                    <div className={styles.topRightContainer}>
                                        <div className={styles.runAutomationContainer}>
                                            <button
                                                type="button"
                                                onClick={async () => {
                                                    if (!setAutomationRunning) return;

                                                    const wasRunning = automationRunning;
                                                    setAutomationRunning(!wasRunning);

                                                    if (wasRunning) {
                                                        if (selectedServerForAutomation) {
                                                            await killAutomation(selectedServerForAutomation);
                                                        } else {
                                                            console.error(
                                                                "No server selected to kill automation."
                                                            );
                                                            toast.error(
                                                                "No server selected to stop automation."
                                                            );
                                                            setAutomationRunning(wasRunning);
                                                        }
                                                    } else {
                                                ...
                                                className={`${styles.runAutomationButton} ${variableData.length === 0 &&
                        ...
                        <div className={styles.tableAndMinimapWrapper}>
                            {" "}
                            <div
                                ref={tableContainerRef}
                                className={styles.tableOverflowContainer}
                            >
                                    ...
                                    <tbody>
                                        {Object.keys(variableRowData).length > 0 ? (
                                            (() => {
                                                ...
                                                tableRowElementsRef.current = new Array(
                                                    maxArrayLength
                                                ).fill(null);
                                                const rowDataMap = new Map<
                                                    number,
                                                    Map<string, tableCellData>
                                                >();
                                                ...
                                                Object.keys(variableRowData).forEach((key) => {
                                                    ...
                                                    if (rowMatch) {
                                                        ...
                                                        rowDataMap
                                                            .get(rowNum)!
                                                            .set(baseKey, variableRowData[key]);
                                                ...
                                                return Array.from({ length: maxArrayLength }).map(
                                                    (_, rowIndex) => {
                                                        ...
                                                        if (
                                                            Array.isArray(systemLogFailed) &&
                                                            systemLogFailed.includes(taskID)
                                                        ) {
                                                            classes.push(styles.rowFailed);
                                                        }

                                                        if (
                                                            Array.isArray(systemLogBatch) &&
                                                            systemLogBatch.includes(taskID)
                                                        ) {
                                                            classes.push(styles.rowBatch);
                                                        }

                                                        if (
                                                            Array.isArray(systemLogSave) &&
                                                            systemLogSave.length > 0
                                                        ) {
                                                            if (
                                                                systemLogSave.length >= 1 &&
                                                                systemLogSave[systemLogSave.length - 1] ===
                                                                taskID
                                                            ) {
                                                                classes.push(styles.rowSavedRecent);
                                                            } else if (
                                                                systemLogSave.length >= 2 &&
                                                                systemLogSave[systemLogSave.length - 2] ===
                                                                taskID
                                                            ) {
                                                                classes.push(styles.rowSavedOlder1);
                                                            } else if (
                                                                systemLogSave.length >= 3 &&
                                                                systemLogSave[systemLogSave.length - 3] ===
                                                                taskID
                                                            ) {
                                                                classes.push(styles.rowSavedOlder2);
                                                            } else if (
                                                                systemLogSave.length >= 4 &&
                                                                systemLogSave[systemLogSave.length - 4] ===
                                                                taskID
                                                            ) {
                                                                classes.push(styles.rowSavedOlder3);
                                                            } else if (
                                                                systemLogSave.length >= 5 &&
                                                                systemLogSave[systemLogSave.length - 5] ===
                                                                taskID
                                                            ) {
                                                        ...
                                                        return (
                                                            <tr
                                                                ...
                                                                ref={(el) => {
                                                                    if (el)
                                                                        tableRowElementsRef.current[rowIndex] = el;
                                                                }}
                                                            >
                                                                ...
                                                                {classKeyInputObjects.map((keyObj) => {
                                                                    ...
                                                                    const cellData = rowDataMap
                                                                        .get(rowIndex)
                                                                        ?.get(keyObj.value);
                                                                    const isCompositeCell =
                                                                        cellData?.isComposite ?? false;
                                                                    const isPackage =
                                                                        cellData?.isPackage ?? false;
                                                                    const cellValue = cellData?.value;
                                                                    let isDefaultCell =
                                                                        cellData?.isDefault ?? false;
                                                                    const isDisabledCell =
                                                                        cellData?.isDisabled ?? false;
                                                                    const isExpanded =
                                                                    ...
                                                                    if (cellData) {
                                                                        if (
                                                                            ...
                                                                            const packageObj =
                                                                                cellValue as PackageObject;
                                                                            let fileCount = 0;
                                                                            let errorMsg = "";
                                                                            if (packageObj.variableData) {
                                                                                if (
                                                                                    packageObj.variableData instanceof Map
                                                                                ) {
                                                                                    packageObj.variableData.forEach(
                                                                                        (entry) => {
                                                                                            if (
                                                                                                entry &&
                                                                                                ...
                                                                                                try {
                                                                                                    ...
                                                                                                    console.error(
                                                                                                        `Error parsing package entry value in Table (${cellKey}):`,
                                                                                                        e,
                                                                                                        entry.value
                                                                                                    );
                                                                                    ...
                                                                                    console.warn(
                                                                                        `Expected packageObj.variableData to be a Map for cell ${cellKey}, but got:`,
                                                                                        typeof packageObj.variableData,
                                                                                        packageObj.variableData
                                                                                    );
                                                                                    ...
                                                                                    Object.values(
                                                                                        ...
                                                                                        if (
                                                                                            ...
                                                                                            try {
                                                                                                ...
                                                                                                console.error(
                                                                                                    `Error parsing package entry value in Table (object fallback for ${cellKey}):`,
                                                                                                    e,
                                                                                                    entry.value
                                                                                                );
                                                                                            }
                                                                                ...
                                                                                console.warn(
                                                                                    `Package object for cell ${cellKey} has no variableData.`
                                                                                );
                                                                                errorMsg = ` Error: No content`;
                                                                            }
                                                                            displayContent = `PKG: ${packageObj.name ||
                                                                                `ID ${packageObj.dataId}`
                                                                                } (${fileCount} files)`;
                                                                            ...
                                                                            if (typeof cellValue === "object") {
                                                                                ...
                                                                                console.warn(
                                                                                    `Cell ${cellKey} has object value but isPackage flag is false or structure is wrong.`
                                                                                );
                                                                            } else {
                                                                                displayContent = "[Invalid Data]";
                                                                                console.warn(
                                                                                    `Unexpected cellValue type for key ${cellKey}:`,
                                                                                    cellValue
                                                                                );
                                                                            }
                                                                    ...
                                                                    return (
                                                                        <td
                                                                            ...
                                                                            className={`${isDefaultCell
                                                                                ? styles.tableCellDefault
                                                                                : styles.tableCell
                                                                                } ${isDisabledCell
                                                                                    ? styles.disabledCell
                                                                                    : ""
                                                                            ...
                                                                            <div className={styles.tableContainer}>
                                                                                ...
                                                                                {isPackage &&
                                                                                    ...
                                                                                    "variableData" in cellValue && (
                                                                                        ...
                                                                                        >
                                                                                            {(() => {
                                                                                                const pkgObjForExpand =
                                                                                                    cellValue as PackageObject;
                                                                                                if (
                                                                                                    !pkgObjForExpand.variableData
                                                                                                )
                                                                                                    return (
                                                                                                        <div
                                                                                                            className={
                                                                                                                styles.packageItemDetail
                                                                                                            }
                                                                                                        >
                                                                                                ...
                                                                                                const entriesToRender: React.ReactNode[] =
                                                                                                    [];
                                                                                                if (
                                                                                                    pkgObjForExpand.variableData instanceof
                                                                                                    Map
                                                                                                ) {
                                                                                                    pkgObjForExpand.variableData.forEach(
                                                                                                        (entry, pkgContentKey) => {
                                                                                                            if (
                                                                                                                entry &&
                                                                                                                typeof entry.value ===
                                                                                                                ...
                                                                                                                try {
                                                                                                                    ...
                                                                                                                    if (
                                                                                                                        ...
                                                                                                                        entriesToRender.push(
                                                                                                                            ...
                                                                                                                            >
                                                                                                                                ...
                                                                                                                                {parsedValue.filename.map(
                                                                                                                                    (
                                                                                                                                        fname: string,
                                                                                                                                        fileIdx: number
                                                                                                                                    ) => (
                                                                                                                                        <div
                                                                                                                                            key={`${cellKey}-pkg-${pkgContentKey}-file-${fileIdx}`}
                                                                                                                                            className={
                                                                                                                                                styles.fileUrlPair
                                                                                                                                            }
                                                                                                                                        >
                                                                                                                    ...
                                                                                                                    console.error(
                                                                                                                        `Error parsing package entry for expand view (${cellKey}):`,
                                                                                                                        e,
                                                                                                                        entry.value
                                                                                                                    );
                                                                                                                    entriesToRender.push(
                                                                                                                        <div
                                                                                                                            key={`${cellKey}-pkg-${pkgContentKey}-error`}
                                                                                                                            className={
                                                                                                                                styles.packageItemDetail
                                                                                                                            }
                                                                                                                        >
                                                                                                    ...
                                                                                                    console.warn(
                                                                                                        `Expected variableData to be a Map for expanded view cell ${cellKey}`
                                                                                                    );
                                                                                                    entriesToRender.push(
                                                                                                        <div
                                                                                                            className={
                                                                                                        ...
                                                                                                        >
                                                                                                            ...
                                                                                                            data structure (not a
                                                                                                            Map).
                                                                                                        </div>
                                                                                                    );
                                                                                                }
                                                                                                return entriesToRender.length >
                                                                                                    0 ? (
                                                                                                    entriesToRender
                                                                                                ) : (
                                                                                                    <div
                                            ...
                                            <tr
                                                className={
                                                    Object.keys(variableData).length > 0
                                                        ? "show-default-unavailable-data"
                                                        : "hide-default-unavailable-data"
                                                }
                                            >
                                                <td
                                                    colSpan={classKeyInputObjects.length}
                            ...
                            {(() => {
                                const currentMaxIndex = getMaxRowIndex(variableRowData);
                                const maxArrayLength = Math.max(1, currentMaxIndex + 1);
                                if (
                                    maxArrayLength > 0 &&
                                    Object.keys(variableRowData).length > 0
                                ) {
                                    return (
                                        <Minimap
                                            totalRows={maxArrayLength}
                                            systemLogCurrent={systemLogCurrent}
                                            systemLogFailed={systemLogFailed}
                                            systemLogBatch={systemLogBatch}
                                            systemLogSave={systemLogSave}
                                            onMinimapRowClick={handleMinimapRowClick}
                                        />
                                    );
                                }
                                return null;
                        ...
                        {!headerOrigin &&
                            ...
                            classKeyInputObjects.length > 0 && (
                                <div className={styles.preOriginTip}>
                                    <p>
                                        Click on a <b>Class Key</b> above to select an <b>Origin</b>{" "}
                                        point for the automation process
                                    </p>
                                </div>
                            )}
                    </div>
...
const ClassKey: React.FC<{
    ...
    disableCellRow,
        ...
        return (
            ...
            >
                {headerOrigin === input ? (
                    <div className={styles.headerOriginDisplay}>ORIGIN</div>
                ) : (
                    ""
                )}
